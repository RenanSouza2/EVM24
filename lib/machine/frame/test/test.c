...
void test_frame_get_jumpdest()
{
    ...
    assert(uint64_vec_immed(jumpdest, 0));
    vec_free(&jumpdest);
    ...
    assert(uint64_vec_immed(jumpdest, 1, 0));
    vec_free(&jumpdest);
...
void test_frame_mstore()
{
    ...
    assert_64(frame_mstore(&f), 2);
    assert(frame_immed(f, IGN, 5, 0, IGN));
    frame_free(&f);
    ...
    assert_64(frame_mstore(&f), 0);
    assert(frame_immed(f, IGN, GAS_DEF - 104, IGN, IGN));
    frame_free(&f);
...
void test_frame_mstore8()
{
    ...
    assert(frame_immed(f, 1, GAS_DEF - 6, 1, WORD(U64_FF, 0, 0, 0), 0));
    frame_free(&f);
    ...
    for(int i=0; i<1024; i++)
        assert(!stack_push_immed(&f.s, W1(i)));
    assert_64(frame_mstore8(&f), 0);
    assert(frame_immed(f, IGN, GAS_DEF - 101, IGN, IGN));
    frame_free(&f);
...
void test_frame_push()
{
    ...
    assert(frame_immed(f, 1, GAS_DEF - 2, IGN, 1, W1(0)));
    frame_free(&f);
    ...
    for(int i=1; i<=32; i++)
    {
        ...
        assert_64(frame_push(&f), 0);
        assert(frame_immed(f, i+1, GAS_DEF - 3, IGN, 1, w));
    ...
    assert(frame_immed(f, IGN, 0, IGN, IGN));
    frame_free(&f);
    ...
    for(int i=0; i<1023; i++)
        assert(!stack_push_immed(&f.s, W1(i)));
    assert_64(frame_push(&f), 0);
    assert(frame_immed(f, IGN, GAS_DEF - 2, IGN, IGN));
    frame_free(&f);
...
void test_frame_return()
{
    ...
    assert(frame_o_immed(fo, true, GAS_DEF, "0x000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"));
    frame_free(&f);
    frame_o_free(&fo);
    ...
    fo = frame_return(&f);
    assert(frame_o_immed(fo, true, GAS_DEF - 3, "0x101112131415161718191a1b1c1d1e1f00000000000000000000000000000000"));
    frame_free(&f);
    frame_o_free(&fo);
    ...
    fo = frame_return(&f);
    assert(frame_o_immed(fo, false, GAS_DEF, "0x"));
    frame_free(&f);
    frame_o_free(&fo);
