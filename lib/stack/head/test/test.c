...
void test_stack_evm_push()
{
    ...
    stack_evm_t s = stack_init();
    assert(stack_evm_push(&s, &w1) == true);
    assert(stack_immed(s, 1, w1));

    printf("\n\t\t%s 2\t\t", __func__);
    word_t w2 = WORD(1, 2, 3, 4);
    assert(stack_evm_push(&s, &w2) == true);
    ...
    for(int i=0; i<1024; i++)
        assert(stack_evm_push(&s, &w) == true);
    assert(stack_evm_push(&s, &w) == false);
    stack_free(&s);
