...
uint64_t rlp_get_size_2(uint64_p head_size, uint64_p body_size, byte_p b, uint64_t _size_size, uint64_t size)
{
    ...
    uint64_t _body_size = uint64_init_byte_vec(&b[1], _size_size);
...
uint64_t rlp_get_size(uint64_p head_size, uint64_p body_size, byte_p b, uint64_t size)
{
    ...
    if(b0 < 248)
    {
        ERR(rlp_get_size_1(head_size, body_size, b0 - 192, size), 6);
        return 0;
    }
...
uint64_t rlp_get_sizes_rec(uint64_vec_p r1_sizes, byte_p b, uint64_t list_size, uint64_t cnt)
{
    if(list_size == 0)
    {
        *r1_sizes = uint64_vec_init_clean(cnt << 1);
        return 0;
    }
    ...
    r1_sizes->arr[index    ] = head_size;
    r1_sizes->arr[index + 1] = body_size;
    return 0;
}
...
uint64_t rlp_decode_rec_l(evm_rlp_p r, byte_p b, uint64_t list_size)
{
    ...
    for(uint64_t i=0, ptr=0; i<r_vec_size; i++)
    {
        ...
        index = i << 1;
        head_size = r1_sizes.arr[index];
        body_size = r1_sizes.arr[index + 1];
...
{
    uint64_t head_size, body_size;
    ERR(rlp_get_size(&head_size, &body_size, b->arr, b->size), 1);
    ...
    evm_rlp_t _r;
    ERR(rlp_decode_rec(&_r, b->arr, head_size, body_size), 3);
